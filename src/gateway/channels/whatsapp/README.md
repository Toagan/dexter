...
Chat with FinancialReports through WhatsApp by linking your phone to the gateway. Messages you send to yourself (self-chat) are processed by FinancialReports and responses are sent back to the same chat.
...
- FinancialReports installed and working (see main [README](../../../../README.md))
- WhatsApp installed on your phone
- Your phone connected to the internet

## 🔗 How to Link WhatsApp

Link your WhatsApp account to FinancialReports by scanning a QR code:
...
After linking, you'll be asked how you want to use FinancialReports:

### Option 1: Self-chat (personal phone)

Use your own WhatsApp to talk to FinancialReports by messaging yourself. The linked phone number is added to `allowFrom` and self-chat mode is activated automatically.

### Option 2: Dedicated bot phone

If FinancialReports has its own phone number (e.g. a separate SIM), choose this option and enter the phone number(s) allowed to message it. The gateway will be configured with `dmPolicy: "allowlist"` so other people can DM the bot.
...
```
[whatsapp] Connected
FinancialReports gateway running. Press Ctrl+C to stop.
```

The gateway will now listen for incoming WhatsApp messages and respond using FinancialReports.
...
4. You'll see a typing indicator while FinancialReports processes
5. FinancialReports's response will appear in the chat

**Example conversation:**
```
You: What was NVIDIA's revenue in 2024?
FinancialReports: NVIDIA's revenue for fiscal year 2024 was $60.9 billion...
```
...
**Bot phone configuration** (dedicated FinancialReports phone, others message it):
```json
{
  "gateway": {
    "accountId": "default",
...
| Setting | Description |
|---------|-------------|
| `channels.whatsapp.allowFrom` | Phone numbers allowed to message FinancialReports (E.164 format) |
...
FinancialReports can participate in WhatsApp group chats, responding only when @-mentioned.
...
| `groupAllowFrom` | Which groups FinancialReports can participate in (`["*"]` for any) |

You don't need to list individual group members — when `groupPolicy` is `"open"`, FinancialReports will respond to @-mentions from anyone in any group it's added to.

### Usage

1. Add FinancialReports's WhatsApp number to a group
2. Send messages normally — FinancialReports stays silent
3. @-mention FinancialReports (tap `@` and select from the picker) to get a response
4. FinancialReports sees recent group messages for context, so it can follow the conversation
...
2. **Unlink from WhatsApp:**
   ...
   - Tap on the FinancialReports device and select **Log Out**
